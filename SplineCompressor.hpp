...
public:
    ...
    static bool sampleSpline(const Point* control, size_t control_size, float* out, size_t sample_count) {
        if (control_size < 2 || sample_count < 2) return false;
        ...
        for (size_t i = 0; i < sample_count; i++) {
            ...
            out[i] = a[idx] + b[idx] * dx + c[idx] * dx * dx + d[idx] * dx * dx * dx;
        }
...
private:
    ...
    static void freeAll(float* a, float* b, float* c, float* d,
